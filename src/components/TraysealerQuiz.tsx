...
export type TraysealerQuizPayload = {
...
const STEPS: Step[] = [
  {
    ...
    options: [
      { emoji: "🥩", label: "Мясо, птица, полуфабрикаты" },
      { emoji: "🐟", label: "Рыба и морепродукты" },
      { emoji: "🧀", label: "Молочная продукция, сыры" },
      { emoji: "🥗", label: "Готовая еда, кулинария, салаты" },
      { emoji: "🍓", label: "Овощи, фрукты, зелень" },
      { emoji: "⚙️", label: "Непищевая продукция", desc: "медицина, техника, комплектующие" },
      { emoji: "🤔", label: "Пока в процессе выбора / запускаю новый продукт" },
    ],
  },
  {
    title: "Сколько лотков в смену нужно запаивать?",
    options: [
      { emoji: "📦", label: "До 200 лотков/смену", desc: "Небольшое производство или стартап" },
      { emoji: "🏬", label: "200–1 000 лотков/смену", desc: "Малое производство, фермерское хозяйство" },
      { emoji: "🏢", label: "1 000–5 000 лотков/смену", desc: "Среднее производство" },
      { emoji: "🏭", label: "Более 5 000 лотков/смену", desc: "Крупное производство, поставки в сети" },
      { emoji: "❓", label: "Пока не знаю", desc: "нужна помощь с расчётом" },
    ],
  },
  {
    title: "Нужна ли упаковка в модифицированной газовой среде (MAP)?",
    options: [
      { emoji: "✅", label: "Да, нужна MAP — хочу продлить срок годности" },
      { emoji: "🔄", label: "Возможно в будущем — хочу аппарат «на вырост»" },
      { emoji: "❌", label: "Нет, достаточно обычной запайки лотка" },
      { emoji: "❓", label: "Не знаю, нужна консультация" },
    ],
  },
  {
    title: "Какое упаковочное оборудование у вас сейчас?",
    options: [
      { emoji: "🚫", label: "Ничего нет — упаковываем вручную или на аутсорсе" },
      { emoji: "📦", label: "Есть бытовой вакууматор / простой запайщик лотков" },
      { emoji: "⚙️", label: "Есть трейсилер, но он устарел / не справляется с объёмами" },
      { emoji: "🏭", label: "Есть линия, нужен дополнительный аппарат или замена" },
    ],
  },
];
...
type Props = {
  ...
  open?: boolean;
  onClose?: () => void;
  onSubmit: (data: TraysealerQuizPayload) => Promise<boolean>;
};

export default function TraysealerQuiz({ variant = "modal", open = true, onClose, onSubmit }: Props) {
      ...
      <div className="px-4 sm:px-6 md:px-8 pb-5 sm:pb-6 overflow-y-auto flex-1">
        {done ? (
          ...
          <div>
            ...
            <p className="text-[#666] text-[14px] mb-5">Подготовим подборку моделей и КП, менеджер свяжется в течение 15 минут</p>
